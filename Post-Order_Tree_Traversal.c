...
void Stack_Traverse(node *ptr) {
	node *ptr_pushed = nBuf[1], *ptr_visited = nBuf[1];

	for (;;) {
		while (ptr != nBuf[1] && ptr != ptr_visited) {
			if (ptr != ptr_pushed)
				Push(ptr);
		...
		if (!IsStackEmpty()) {
			...
			if (ptr->left != nBuf[1] && ptr->right == nBuf[1] && ptr->left != ptr_visited) {
				Push(ptr);
				ptr = ptr->left;
			}

			if (ptr->right == nBuf[1] || ptr->right == ptr_visited) {
				Display(ptr);
				ptr_visited = ptr;
			}
		}
